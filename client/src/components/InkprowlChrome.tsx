...
export function FloatingPlayer() {
  ...
  return (
    <div className={`floating-player ${minimized ? "is-minimized" : ""}`} aria-label="INKPROWL music player" onPointerDown={beginDrag} onPointerMove={moveDrag} onPointerUp={endDrag} onPointerCancel={endDrag} style={{ transform: `translate(${position.x}px, ${position.y}px)` }}>
      ...
      {playlistOpen && !minimized && <div className="player-playlist" role="dialog" aria-label="Current playlist"><span>NOW PLAYING</span><strong>{siteMedia.soundtrackTitle || "Curated sound"}</strong><small>{soundtrackReady ? "Cloudinary audio edition" : "Awaiting an owner upload"}</small></div>}
    </div>
  );
}
