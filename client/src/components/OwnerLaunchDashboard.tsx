...
export function OwnerLaunchDashboard({ connection, requestAuthorization, onLogout, onOpenAdvanced }: { connection: OwnerConnection | null; requestAuthorization: () => void; onLogout: () => void; onOpenAdvanced: (panel: "video" | "brand" | "ads") => void }) {
  ...
  return <main className="owner-launch-dashboard" data-workspace={workspace} aria-label="INKPROWL media publishing dashboard">
    ...
    <div className="owner-upload-grid">
      ...
      <article className="owner-upload-card"><div className="owner-upload-icon"><Music2 size={22} /></div><span className="eyebrow">FLOATING MUSIC PLAYER</span><h4>Song Upload & Publish</h4><p>Upload one MP3, WAV, M4A, or OGG file for the public movable music player.</p><label className="launch-file-picker"><input type="file" accept={ownerUploadAccept.soundtrack} onChange={(event) => chooseSingleMedia("soundtrack", event, setSongFile, setSongTitle)} /><span>{songFile?.name ?? "Choose your song file"}</span></label><label>Song title <input value={songTitle} onChange={(event) => setSongTitle(event.target.value)} placeholder="Auto-generated from filename" /></label><div className="launch-spacer" /><button type="button" className="admin-primary-action" onClick={() => void publish("soundtrack", songFile ? [songFile] : [], songTitle)}><Music2 size={16} /> Upload & Publish song</button></article>
      ...
      <article className="owner-upload-card owner-brand-upload-card"><div className="owner-upload-icon"><ImagePlus size={22} /></div><span className="eyebrow">BRAND STUDIO</span><h4>Hero Banner Upload & Publish</h4><p>Choose the homepage hero banner from your device. PNG, JPG, WebP, or AVIF is queued directly for permanent Cloudinary delivery.</p><label className="launch-file-picker"><input type="file" accept={ownerUploadAccept["hero-banner"]} onChange={(event) => chooseSingleMedia("hero-banner", event, setHeroBannerFile, setHeroBannerTitle)} /><span>{heroBannerFile?.name ?? "Choose hero banner file"}</span></label><label>Banner label <input value={heroBannerTitle} onChange={(event) => setHeroBannerTitle(event.target.value)} placeholder="Auto-generated from filename" /></label><div className="launch-spacer" /><button type="button" className="admin-primary-action" onClick={() => void publish("hero-banner", heroBannerFile ? [heroBannerFile] : [], heroBannerTitle)}><UploadCloud size={16} /> Upload & Publish hero banner</button></article>
    </div>
