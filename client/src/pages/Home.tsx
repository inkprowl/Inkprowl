...
export default function Home() {
  ...
  return (
    <PageFrame dark>
      ...
      {stageVideoUrl && <section className="media-section section-wrap"><div className="section-heading inverse"><div><span className="eyebrow light">{sponsorFilmIsHeroFallback ? sponsoredCampaign.label : "IN MOTION"}</span><h2>{sponsorFilmIsHeroFallback ? <>{sponsoredCampaign.clientName}<br />in the <em>prowl.</em></> : <>A moving image,<br />beneath the ink.</>}</h2></div><p>{sponsorFilmIsHeroFallback ? (sponsoredCampaign.clientUrl ? "Sponsored content. The visit control opens the approved client website in a new tab." : "Sponsored content preview. Add an approved client destination in the owner admin to enable the visit control.") : "A Cloudinary-hosted studio reel or seasonal visual essay."}</p></div><div className="video-stage"><CloudinaryVideoPlayer className="hero-video" src={stageVideoUrl} title={stageVideoTitle} clientUrl={sponsorFilmIsHeroFallback ? sponsoredCampaign.clientUrl : undefined} clientName={sponsorFilmIsHeroFallback ? sponsoredCampaign.clientName : undefined} /><div className="video-note">{sponsorFilmIsHeroFallback ? "This sponsor film occupies the primary video stage until an INKPROWL studio reel is published." : "Cloudinary-hosted studio reel."}</div></div></section>}
      {sponsoredCampaign.enabled && sponsoredCampaign.videoUrl && siteMedia.heroFilmUrl && <section className="sponsor-film-section section-wrap"><div className="section-heading"><div><span className="eyebrow">{sponsoredCampaign.label}</span><h2>{sponsoredCampaign.clientName}</h2></div><p>{sponsoredCampaign.clientUrl ? "Sponsored content. The visit control opens the approved client website in a new tab." : "Sponsored content preview. Add an approved client destination in the owner admin to enable the visit control."}</p></div><CloudinaryVideoPlayer className="sponsor-video" src={sponsoredCampaign.videoUrl} title={`${sponsoredCampaign.clientName} sponsored film`} clientUrl={sponsoredCampaign.clientUrl} clientName={sponsoredCampaign.clientName} /></section>}
